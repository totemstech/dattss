...
var process = function(spec, my) {
  ...
  agg = function(data) {
    ...
    ['c', 'g', 'ms'].forEach(function(typ) {
      ...
      data.prt[typ].forEach(function(prt) {
        my.prts[typ][prt.nam] = my.prts[type][prt.nam] || [];
        prt.rcv = my.last;
        prt.drt = true;
      ...
      for(var st in my.prts[typ]) {
        while(my.prts[typ][st].length > 0 && 
              ...
              (my.last - my.prts[typ][st][0].rcv) > my.cfg['DATTSS_USER_CROND_PERIOD']) {
      ...
      for(var st in my.prts[typ]) {
        ...
        for(var i = my.cur.sts[typ].length - 1; i >= 0; i--) {
          if((my.last - my.cur.sts[typ][i].dte) > my.cfg['DATTSS_STAT_EVICTION_PERIOD']) {
  ...
  commit = function() {
    ...
    mplex.go(function() {
      // write back current
      fs.writeFile(curr_dir + '/' + my.name '.cur', JSON.stringify(my.cur), 'utf8', function(err) {
      ...
      ['c', 'g', 'ms'].forEach(function(typ) {
        ...
        for(var st in my.prts[typ]) {
          ...
          var agg = agg_partials(my.prts[typ][st]);
