 ...
 *
 * @param {cfg, user, process, type, name}
 */
    ...
    for(var i = 0; i < total; i ++) {
      ...
      if(my.data[min_str]) {
        if(day_str === today_str) {
          agg.today[pos] = my.data[min_str];
        }
        else {
          agg.past[pos] = agg.past[pos] || [];
          agg.past[pos].push(my.data[min_str]);
        }
      }
    }

    agg.past.forEach(function(prts, i) {
      agg.past[i] = partials.agg_partials(prts);
    });
